  ...
  <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
            ...
            data.forEach((deliveryPoint) => {
                try {
                    let qrcodeElement;
                    if(view === 'mobile') {
                        qrcodeElement = document.getElementById(`qrcodeMobile${deliveryPoint.id}`);
                    } else if (view === 'desktop') {
                        qrcodeElement = document.getElementById(`qrcodeDesktop${deliveryPoint.id}`);
                    }
        ...
        fetch(`ajaxController.php?q=record&id=${id}&token=${token}`, {method: 'GET'}).then(async (res) => {
            ...
            data.forEach((record) => {
                ...
                if (record.name === null) {
                    ...
                    console.log('setname', this.recordName.value);
                ...
                if (record.deliverer === null) {
                    this.recordDeliverer.value = 'Not Provided';
                } else {
                ...
                if (record.status === null) {
                    ...
                    this.recordStatus.innerHTML += `<option value="${record.statusCode}">${record.status}</option>`;
                }
                if (record.photo === null) {
                    this.recordPhoto.filename = null;
                } else {
            ...
            this.recordDeliverer.innerHTML = '';
            deliverers.forEach((deliver) => {
                this.recordDeliverer.innerHTML += `<option value="${deliver.id}">${deliver.username}</option>`
            })
    ...
    editRecord(token) { //Used for updating records
        ...
        if(this.formData.deliverer !== parseInt(this.recordDeliverer.value)) {
        ...
        if(this.formData.photo !== this.recordPhoto.filename) {
            newFormData.photo = this.recordPhoto.filename;
        }
    ...
    async setDeliverers(recordDeliverer) {
        let deliverers = await this.getDeliverers();
    ...
    async setStatusTypes(recordStatus) {
        let statusTypes = await this.getStatusTypes();
    ...
    setUpCreateForm() {
        this.setStatusTypes(this.recordStatus).then(r => {});
        this.setDeliverers(this.recordDeliverer).then(r => {});
        this.recordID.value = '';
